...
azimuth_deg_dist = np.random.uniform(0,360,views_amount)
...
rho_dist = np.random.uniform(0.15,0.3,views_amount)
...
for i in range(0, views_amount):
    ...
    elevation_deg = round(360 - abs(elevation_deg_dist[i]) if elevation_deg_dist[i] < 0 else elevation_deg_dist[i])
    theta_deg = round(360 - abs(theta_deg_dist[i]) if theta_deg_dist[i] < 0 else theta_deg_dist[i])
    rho = rho_dist[i]

    if azimuth_deg > 360 or elevation_deg > 360 or theta_deg > 360:
